...
def lazyPRMVisualize(planner, solution = [] , ax=None, nodeSize = 300):
    ...
    pos_unnown_dim = nx.get_node_attributes(graph,'pos')
    first_pos = next(iter(pos_unnown_dim.values()))
    num_dimensions = len(first_pos)

    if num_dimensions > 2:
        pos = {node: (coords[0], coords[1]) for node, coords in pos_unnown_dim.items()}
    else:
        pos = pos_unnown_dim
...
def visibilityPRMVisualizeWspace(planner, solution, ax = None, nodeSize = 100):
